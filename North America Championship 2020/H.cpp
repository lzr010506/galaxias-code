...
int main()
{
	rep0(i, 3) scanf("%s", ch[i]);
	int n = strlen(ch[0]);
	rep0(i, n)
		rep0(j, n)
		{
			//1、2
			if(ch[0][j] == ch[1][(i + j) % n]) ok[0][i] = 1;
			//2
			if(ch[0][j] == ch[2][(i + j) % n]) ok[1][i] = 1;
			//0,1
			if(ch[1][j] == ch[2][(i + j) % n]) ok[2][i] = 1;
		}
	int ans = 10 * n;
	rep0(i, n)
		rep0(j, n)
			if(!ok[1][i] && !ok[0][j] && !ok[2][(i + n - j) % n])
			{
				int now = max(i, j);
				now = min(max(n - i, n - j), now);
				now = min(min(i, n - i) + min(j, n - j), now);
				ans = min(ans, now);
			}
