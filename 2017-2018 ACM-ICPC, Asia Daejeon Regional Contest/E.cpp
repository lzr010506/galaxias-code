...
int main() {
	...
	for(int i=0;i<V.size();i++) {
		dinic.init();
		dinic.n = n, dinic.m = m, dinic.s = V[i].u, dinic.t = V[i].v;
		for(int j=0;j<i;j++) {
			if(V[j].val<V[i].val) {
				dinic.addedge(V[j].u,V[j].v,1);
				dinic.addedge(V[j].v,V[j].u,1);
			}
		}
		ans += dinic.Maxflow(dinic.s,dinic.t);
	}
	cout << ans << endl;
}