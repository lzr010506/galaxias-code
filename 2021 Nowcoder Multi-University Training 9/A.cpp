...
int S(int n, int m) {
    ...
    for (int i = 0, v = 1; i <= m + 1; i++, (v = 1LL * v * n % M))
    {
        s = (s + 1LL * A[m][i] * v % M);
        s %= M;
    }
...
int solve(int k1, int k2, int a, int b, int c, int n, int dep) {
    ...
    if (a == 0)
        return f[dep][k1][k2] = 1LL * S(n, k1) * ksm(b / c, k2) % M;
    ...
    if (a >= c || b >= c) {
        for (int i = 0; i <= k2; i++)
            for (int j = 0; j <= k2 - i; j++) {
                ...
                v = v + 1LL * C[k2][i] * C[k2 - i][j] % M * ksm(a / c, i) % M * ksm(b / c, j) % M * solve(k1 + i, k, a % c, b % c, c,
                        n, dep + 1) % M;
                if(v >= M) v -= M;
            }
    ...
    int m = (1LL * a * n + b) / c - 1;
    v = 1LL * S(n, k1) * ksm(m + 1, k2) % M;
  
    for (int i = 0; i <= k2 - 1; i++)
        for (int j = 0; j <= k1 + 1; j++)
        {
            (v += M - 1LL * C[k2][i] * A[k1][j] % M * solve(i, j, c, c - b - 1, a, m, dep + 1) % M) %= M;
        }
...
void get_A(int n , int *y) {
    for (int i = 1 ; i <= n + 1 ; ++i) {
        ...
        for (int j = 1 ; j <= n + 1 ; ++j) {
            ...
            int ned = (mod - j) % mod ;
            for (int k = n ; k ; --k) g[k] = ((ll)g[k] * ned % mod + g[k - 1]) % mod ;
            g[0] = (ll)g[0] * ned % mod ;
        }
...
signed main() {
    ...
    for (int i = 0 ; i <= q + 1 ; ++i) {
        memset(f, -1, sizeof f);
        int tmp = (ll)K[i] * solve(p , i , a , b , c , n , 0) % mod ;
        (res += tmp) %= mod ;
    }
    printf("%d\n" , res) ;
}